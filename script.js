      ...
      if (window.zappyContactFormLoaded) {
        try {
          ...
          const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '09e2cee6-c9d5-4e8e-8643-87f91e00294a',
              name: formData.name,
              email: formData.email,
              subject: formData.company ? `Contact Form - ${formData.company}` : 'Contact Form Submission',
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Minimal משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
